...
internal class TaskScheduler {
    ...
    private suspend fun executeTask(taskNode: TaskNode) {
        ...
        try {
            ...
            Log.d(TAG, "任务执行完成: ${task.name}, 执行时长: ${executionTime}ms, 开始总时长: ${(taskEndTime - SimpleLauncher.getInstance().startTime)}ms")
    ...
    private fun printTaskExecutionTimes() {
        ...
        sb.appendLine("#所有任务合并执行时长:${System.currentTimeMillis() - SimpleLauncher.getInstance().startTime}ms")
        sb.appendLine("#所有任务总时长: ${totalTime}ms")
        sb.appendLine("#各任务执行时间:")

        sortedTasks.forEachIndexed { index, entry ->
            val percentage = String.format(Locale.getDefault(),"%.1f", entry.value * 100.0 / totalTime)
