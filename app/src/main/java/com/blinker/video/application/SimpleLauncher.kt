 ...
 * 启动框架，用于优化启动速度
 */
class SimpleLauncher private constructor() {
    companion object {
        private const val TAG = "SimpleLaunchTask"

        /**
         * 单例实例
         */
        @Volatile
        private var INSTANCE: SimpleLauncher? = null

        /**
         * 获取单例实例
         */
        fun getInstance(): SimpleLauncher {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: SimpleLauncher().also { INSTANCE = it }
            }
        }
    }
     ...
     * 初始化
     */
    fun init(context: Context): SimpleLauncher {
        appContext = context.applicationContext
        Log.d(TAG, "SimpleLaunch 初始化完成")
        return this
    }
     ...
     * 添加任务
     */
    fun addTask(task: Task): SimpleLauncher {
     ...
     * 添加多个任务
     */
    fun addTasks(tasks: List<Task>): SimpleLauncher {
     ...
     */
    fun addTasksCompletedListener(tasks: List<Class<out Task>>, listener: () -> Unit): SimpleLauncher {
        // 检查任务是否已经完成
     ...
     * @param listener 任务完成时的回调
     */
    fun addTaskCompletedListener(taskClass: Class<out Task>, listener: () -> Unit): SimpleLauncher {
     ...
     */
    fun addAllTasksCompletedListener(listener: () -> Unit): SimpleLauncher {
        // 获取所有任务
